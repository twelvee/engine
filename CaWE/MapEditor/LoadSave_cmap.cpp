...
MapFaceT MapFaceT::Create_cmap(TextParserT& TP, EditorMatManT& MatMan)
{
    ...
    if (MapFileVersion<=6)
    {
        ...
        Face.m_PlanePoints[0]/=static_cast<float>(CAFU_ENG_SCALE);
        Face.m_PlanePoints[1]/=static_cast<float>(CAFU_ENG_SCALE);
        Face.m_PlanePoints[2]/=static_cast<float>(CAFU_ENG_SCALE);
    }
...
void MapBezierPatchT::Load_cmap(TextParserT& TP, MapDocumentT& MapDoc, bool IgnoreGroups)
{
    ...
    for (unsigned long y=0; y<cv_Height; y++)
    {
        for (unsigned long x=0 ; x<cv_Width; x++)
        {
            ...
            if (MapFileVersion<=6) Pos/=static_cast<float>(CAFU_ENG_SCALE);
            SetCvPos(x, y, Pos);
...
void EntPropertyT::Load_cmap(TextParserT& TP)
{
    ...
    {
        const Vector3fT Origin=GetVector3f()/static_cast<float>(CAFU_ENG_SCALE);
