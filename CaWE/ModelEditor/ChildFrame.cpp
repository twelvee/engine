...
BEGIN_EVENT_TABLE(ModelEditor::ChildFrameT, wxMDIChildFrame)
    ...
    EVT_MENU_RANGE(ID_MENU_VIEW_AUIPANE_JOINTS_HIERARCHY, ID_MENU_VIEW_LOAD_DEFAULT_PERSPECTIVE,
                   ModelEditor::ChildFrameT::OnMenuView)
    EVT_UPDATE_UI_RANGE(ID_MENU_VIEW_AUIPANE_JOINTS_HIERARCHY, ID_MENU_VIEW_LOAD_DEFAULT_PERSPECTIVE,
                        ModelEditor::ChildFrameT::OnMenuViewUpdate)
    EVT_MENU_RANGE(ID_MENU_MODEL_ANIM_SKIP_BACKWARD, ID_MENU_MODEL_UNLOAD_SUBMODELS,
                   ModelEditor::ChildFrameT::OnMenuModel)
    EVT_UPDATE_UI_RANGE(ID_MENU_MODEL_ANIM_SKIP_BACKWARD, ID_MENU_MODEL_UNLOAD_SUBMODELS,
                        ModelEditor::ChildFrameT::OnMenuModelUpdate)
...
ModelEditor::ChildFrameT::ChildFrameT(ParentFrameT *Parent, const wxString &FileName, ModelDocumentT *ModelDoc)
    : wxMDIChildFrame(Parent, wxID_ANY, FileName, wxDefaultPosition, wxSize(800, 600),
                      wxDEFAULT_FRAME_STYLE | wxMAXIMIZE),
      m_FileName(FileName),
      // Must use a fixed size in place of wxDefaultSize, see <http://trac.wxwidgets.org/ticket/12490> for details.
      m_ModelDoc(ModelDoc),
      m_History(),
    ...
    ViewMenu->AppendCheckItem(ID_MENU_VIEW_AUIPANE_JOINTS_HIERARCHY, "Joints Hierarchy",
                              "Show or hide the joints hierarchy (the skeleton)");
    ViewMenu->AppendCheckItem(ID_MENU_VIEW_AUIPANE_JOINT_INSPECTOR, "Joint Inspector",
                              "Show or hide the joint inspector");
    ...
    ViewMenu->AppendCheckItem(ID_MENU_VIEW_AUIPANE_GUIFIXTURES_LIST, "GUI Fixtures List",
                              "Show or hide the GUI fixtures list");
    ViewMenu->AppendCheckItem(ID_MENU_VIEW_AUIPANE_GUIFIXTURE_INSPECTOR, "GUI Fixture Inspector",
                              "Show or hide the GUI fixture inspector");
    ViewMenu->AppendCheckItem(ID_MENU_VIEW_AUIPANE_ANIMS_LIST, "Animations List", "Show or hide the animations list");
    ViewMenu->AppendCheckItem(ID_MENU_VIEW_AUIPANE_ANIM_INSPECTOR, "Animation Inspector",
                              "Show or hide the animation inspector");
    ViewMenu->AppendCheckItem(ID_MENU_VIEW_AUIPANE_CHANNELS_LIST, "Channels List",
                              "Show or hide the animation channels list");
    ViewMenu->AppendCheckItem(ID_MENU_VIEW_AUIPANE_CHANNEL_INSPECTOR, "Channel Inspector",
                              "Show or hide the animation channel inspector");
    ViewMenu->AppendSeparator();
    ViewMenu->AppendCheckItem(ID_MENU_VIEW_AUIPANE_SCENE_SETUP, "Scene Setup",
                              "Show or hide the scene setup inspector");
    ViewMenu->AppendCheckItem(ID_MENU_VIEW_AUIPANE_SUBMODELS_LIST, "Submodels List", "Show or hide the submodels list");
    ViewMenu->AppendCheckItem(ID_MENU_VIEW_AUIPANE_DLOD_MODELS_LIST, "LoD Models",
                              "Show or hide the list of models for reduced levels of detail");
    ViewMenu->AppendCheckItem(ID_MENU_VIEW_AUIPANE_TRANSFORM_DIALOG, "Transform Dialog",
                              "Show or hide the model transform dialog");
    ViewMenu->AppendSeparator();
    ViewMenu->Append(ID_MENU_VIEW_LOAD_USER_PERSPECTIVE, "&Load user window layout",
                     "Loads the user defined window layout");
    ViewMenu->Append(ID_MENU_VIEW_SAVE_USER_PERSPECTIVE, "&Save user window layout", "Saves the current window layout");
    ViewMenu->Append(ID_MENU_VIEW_LOAD_DEFAULT_PERSPECTIVE, "Load &default window layout",
    ...
    ModelMenu->Append(ID_MENU_MODEL_ANIM_IMPORT, "Import animation sequences...",
                      "Imports additional animation sequences into the model");
    ModelMenu->Append(ID_MENU_MODEL_CHANNEL_ADD, "Add channel", "Adds a new animation channel to the model");
    ModelMenu->Append(-1, "Run benchmark", "Move the camera along a predefined path and determine the time taken")->
            Enable(false);
    ModelMenu->AppendSeparator();
    ModelMenu->Append(ID_MENU_MODEL_LOAD_SUBMODEL, "&Load submodel...",
                      "Loads a submodel (such as a weapon) to show with the main model");
    ...
    m_SkinInspector = new wxStaticText(this, wxID_ANY,
                                       "\nSkins are used to assign alternative materials to the meshes in the model.\n\nTo use a skin, select it in the Skins list, then use the Mesh Inspector to assign a material to the mesh in the selected skin.",
                                       wxDefaultPosition, wxSize(260, 180));
    ...
    m_DlodModelsList = new wxStaticText(this, wxID_ANY, GetLodModelsString(), wxDefaultPosition, wxSize(260, 180),
    ...
    ToolbarDocument->AddTool(ParentFrameT::ID_MENU_FILE_NEW_MODEL, "New",
                             wxArtProvider::GetBitmap(wxART_NEW, wxART_TOOLBAR), "Create a new file");
    ToolbarDocument->AddTool(ParentFrameT::ID_MENU_FILE_OPEN, "Open",
                             wxArtProvider::GetBitmap(wxART_FILE_OPEN, wxART_TOOLBAR), "Open an existing file");
    ToolbarDocument->AddTool(ID_MENU_FILE_SAVE, "Save", wxArtProvider::GetBitmap(wxART_FILE_SAVE, wxART_TOOLBAR),
                             "Save the file");
    ToolbarDocument->AddTool(ID_MENU_FILE_SAVEAS, "Save as",
                             wxArtProvider::GetBitmap(wxART_FILE_SAVE_AS, wxART_TOOLBAR),
                             "Save the file under a different name");
    ToolbarDocument->AddSeparator();
    ToolbarDocument->AddTool(wxID_UNDO, "Undo", wxArtProvider::GetBitmap(wxART_UNDO, wxART_TOOLBAR),
                             "Undo the last action");
    ToolbarDocument->AddTool(wxID_REDO, "Redo", wxArtProvider::GetBitmap(wxART_REDO, wxART_TOOLBAR),
    ...
    AnimToolbar->AddTool(ID_MENU_MODEL_ANIM_SKIP_BACKWARD, "Skip backward",
                         wxArtProvider::GetBitmap("media-skip-backward", wxART_TOOLBAR),
                         "Select previous animation sequence");
    AnimToolbar->AddTool(ID_MENU_MODEL_ANIM_PLAY, "Play Anim",
                         wxArtProvider::GetBitmap("media-playback-start", wxART_TOOLBAR),
                         "Playback the animation sequence", wxITEM_RADIO);
    AnimToolbar->AddTool(ID_MENU_MODEL_ANIM_PAUSE, "Pause Anim",
                         wxArtProvider::GetBitmap("media-playback-pause", wxART_TOOLBAR),
                         "Pause/stop the animation sequence", wxITEM_RADIO);
    AnimToolbar->AddTool(ID_MENU_MODEL_ANIM_SKIP_FORWARD, "Skip forward",
                         wxArtProvider::GetBitmap("media-skip-forward", wxART_TOOLBAR),
                         "Select next animation sequence");
    AnimToolbar->AddSeparator();
    AnimToolbar->AddTool(ID_MENU_MODEL_TRANSFORM, "Transform",
                         wxArtProvider::GetBitmap("transform-rotate-right", wxART_TOOLBAR), "Transform model");
    // AnimToolbar->AddTool(ID_MENU_MODEL_SKIN_ADD,       "Add skin",        wxArtProvider::GetBitmap("window-new", wxART_TOOLBAR), "Add skin");
    AnimToolbar->AddTool(ID_MENU_MODEL_GUIFIXTURE_ADD, "Add GUI fixture",
                         wxArtProvider::GetBitmap("window-new", wxART_TOOLBAR), "Add GUI fixture");
    ...
    m_AUIManager.LoadPerspective(
        wxConfigBase::Get()->Read("ModelEditor/AUI_UserLayout", m_AUIManager.SavePerspective()));
...
bool ModelEditor::ChildFrameT::SubmitNewSkin() {
    ...
    while (Skin.RenderMaterials.Size() < m_ModelDoc->GetModel()->GetMeshes().Size())
        Skin.RenderMaterials.
                PushBack(NULL);
...
bool ModelEditor::ChildFrameT::SubmitImportAnims() {
    ...
    for (size_t PathNr = 0; PathNr < Paths.GetCount(); PathNr++) {
        try {
            ...
            ArrayT<CafuModelT::AnimT> Anims = Importer.Import(m_ModelDoc->GetModel()->GetJoints(),
                                                              m_ModelDoc->GetModel()->GetMeshes());
...
void ModelEditor::ChildFrameT::SaveMaterials(const wxString &OldBaseName, const wxString &BaseName) {
    ...
    if (EditorCmatFile.is_open()) {
        const std::map<std::string, MaterialT *> &Materials = m_ModelDoc->GetModel()->GetMaterialManager().
                GetAllMaterials();
        bool IsFirst = true;
...
bool ModelEditor::ChildFrameT::Save(bool AskForFileName) {
    ...
    if (AskForFileName || FileName == "" || FileName == "New Model" || !FileName.EndsWith(".cmdl") ||
        ...
        wxFileDialog SaveFileDialog(this, // parent
                                    ...
                                    (FN.IsOk() && wxDirExists(FN.GetPath())) ? FN.GetPath() : LastUsedDir,
                                    // default dir
    ...
    if (wxFileExists(FileName) && !wxCopyFile(FileName, FileName + "_bak")) {
        wxMessageBox(
            wxString("Sorry, creating a backup of file '") + FileName + "' at '" + FileName + "_bak" +
            "' did not work out.\n"
    ...
    if (!ModelFile.is_open()) {
        wxMessageBox(
            wxString("CaWE was unable to open the file \"") + FileName +
            "\" for writing. Please verify that the file is writable and that the path exists.");
        return false;
    }
    ...
    wxFileName OldFileName(m_FileName);
    OldFileName.Normalize(
        wxPATH_NORM_DOTS | wxPATH_NORM_ABSOLUTE | wxPATH_NORM_TILDE | wxPATH_NORM_LONG,
        wxEmptyString,
        wxPATH_NATIVE
    );
    ...
    if (OldFileName.FileExists() && wxFileName::DirName(OldFileName.GetPath()) != wxFileName::DirName(
            NewFileName.GetPath())) {
        ...
        if (AllFiles.GetCount() > 0) {
            ...
            for (int i = 0; i < Result; i++) {
                const wxFileName from(
                    OldFileName.GetPath(wxPATH_GET_VOLUME | wxPATH_GET_SEPARATOR) + AllFiles[SelIndices[i]]);
                const wxFileName to(
                    NewFileName.GetPath(wxPATH_GET_VOLUME | wxPATH_GET_SEPARATOR) + AllFiles[SelIndices[i]]);
...
void ModelEditor::ChildFrameT::OnMenuEdit(wxCommandEvent &CE) {
    ...
    if (CE.GetId() == wxID_CUT || CE.GetId() == wxID_DELETE) {
        CommandDeleteT *DelCmd = new CommandDeleteT(m_ModelDoc, m_LastUsedType,
                                                    m_ModelDoc->GetSelection(m_LastUsedType));
        bool Result = DelCmd->Do();

        if (DelCmd->GetMessage() != "") wxMessageBox(DelCmd->GetMessage(), "Delete");
        if (Result) SubmitCommand(DelCmd);
        else delete DelCmd;
    }
}
...
void ModelEditor::ChildFrameT::OnMenuEditUpdate(wxUpdateUIEvent &UE) {
    ...
    switch (UE.GetId()) {
        case wxID_CUT: UE.Enable(false);
            break;
        case wxID_COPY: UE.Enable(false);
            break;
        case wxID_PASTE: UE.Enable(false);
            break;
        case wxID_DELETE: UE.Enable(HaveSel && m_LastUsedType != JOINT);
            break;
    }
}
...
void ModelEditor::ChildFrameT::OnMenuView(wxCommandEvent &CE) {
    switch (CE.GetId()) {
        case ID_MENU_VIEW_AUIPANE_JOINTS_HIERARCHY: PaneToggleShow(m_AUIManager.GetPane(m_JointsHierarchy));
            break;
        case ID_MENU_VIEW_AUIPANE_JOINT_INSPECTOR: PaneToggleShow(m_AUIManager.GetPane(m_JointInspector));
            break;
        case ID_MENU_VIEW_AUIPANE_MESHES_LIST: PaneToggleShow(m_AUIManager.GetPane(m_MeshesList));
            break;
        case ID_MENU_VIEW_AUIPANE_MESH_INSPECTOR: PaneToggleShow(m_AUIManager.GetPane(m_MeshInspector));
            break;
        case ID_MENU_VIEW_AUIPANE_ANIMS_LIST: PaneToggleShow(m_AUIManager.GetPane(m_AnimsList));
            break;
        case ID_MENU_VIEW_AUIPANE_ANIM_INSPECTOR: PaneToggleShow(m_AUIManager.GetPane(m_AnimInspector));
            break;
        case ID_MENU_VIEW_AUIPANE_CHANNELS_LIST: PaneToggleShow(m_AUIManager.GetPane(m_ChannelsList));
            break;
        case ID_MENU_VIEW_AUIPANE_CHANNEL_INSPECTOR: PaneToggleShow(m_AUIManager.GetPane(m_ChannelInspector));
            break;
        case ID_MENU_VIEW_AUIPANE_SKINS_LIST: PaneToggleShow(m_AUIManager.GetPane(m_SkinsList));
            break;
        case ID_MENU_VIEW_AUIPANE_SKIN_INSPECTOR: PaneToggleShow(m_AUIManager.GetPane(m_SkinInspector));
            break;
        case ID_MENU_VIEW_AUIPANE_GUIFIXTURES_LIST: PaneToggleShow(m_AUIManager.GetPane(m_GuiFixturesList));
            break;
        case ID_MENU_VIEW_AUIPANE_GUIFIXTURE_INSPECTOR: PaneToggleShow(m_AUIManager.GetPane(m_GuiFixtureInspector));
            break;
        case ID_MENU_VIEW_AUIPANE_SCENE_SETUP: PaneToggleShow(m_AUIManager.GetPane(m_ScenePropGrid));
            break;
        case ID_MENU_VIEW_AUIPANE_SUBMODELS_LIST: PaneToggleShow(m_AUIManager.GetPane(m_SubmodelsPanel));
            break;
        case ID_MENU_VIEW_AUIPANE_DLOD_MODELS_LIST: PaneToggleShow(m_AUIManager.GetPane(m_DlodModelsList));
            break;
        case ID_MENU_VIEW_AUIPANE_TRANSFORM_DIALOG: PaneToggleShow(m_AUIManager.GetPane(m_TransformDialog));
            break;
...
void ModelEditor::ChildFrameT::OnMenuViewUpdate(wxUpdateUIEvent &UE) {
    switch (UE.GetId()) {
        case ID_MENU_VIEW_AUIPANE_JOINTS_HIERARCHY: UE.Check(m_AUIManager.GetPane(m_JointsHierarchy).IsShown());
            break;
        case ID_MENU_VIEW_AUIPANE_JOINT_INSPECTOR: UE.Check(m_AUIManager.GetPane(m_JointInspector).IsShown());
            break;
        case ID_MENU_VIEW_AUIPANE_MESHES_LIST: UE.Check(m_AUIManager.GetPane(m_MeshesList).IsShown());
            break;
        case ID_MENU_VIEW_AUIPANE_MESH_INSPECTOR: UE.Check(m_AUIManager.GetPane(m_MeshInspector).IsShown());
            break;
        case ID_MENU_VIEW_AUIPANE_ANIMS_LIST: UE.Check(m_AUIManager.GetPane(m_AnimsList).IsShown());
            break;
        case ID_MENU_VIEW_AUIPANE_ANIM_INSPECTOR: UE.Check(m_AUIManager.GetPane(m_AnimInspector).IsShown());
            break;
        case ID_MENU_VIEW_AUIPANE_CHANNELS_LIST: UE.Check(m_AUIManager.GetPane(m_ChannelsList).IsShown());
            break;
        case ID_MENU_VIEW_AUIPANE_CHANNEL_INSPECTOR: UE.Check(m_AUIManager.GetPane(m_ChannelInspector).IsShown());
            break;
        case ID_MENU_VIEW_AUIPANE_SKINS_LIST: UE.Check(m_AUIManager.GetPane(m_SkinsList).IsShown());
            break;
        case ID_MENU_VIEW_AUIPANE_SKIN_INSPECTOR: UE.Check(m_AUIManager.GetPane(m_SkinInspector).IsShown());
            break;
        case ID_MENU_VIEW_AUIPANE_GUIFIXTURES_LIST: UE.Check(m_AUIManager.GetPane(m_GuiFixturesList).IsShown());
            break;
        case ID_MENU_VIEW_AUIPANE_GUIFIXTURE_INSPECTOR: UE.Check(m_AUIManager.GetPane(m_GuiFixtureInspector).IsShown());
            break;
        case ID_MENU_VIEW_AUIPANE_SCENE_SETUP: UE.Check(m_AUIManager.GetPane(m_ScenePropGrid).IsShown());
            break;
        case ID_MENU_VIEW_AUIPANE_SUBMODELS_LIST: UE.Check(m_AUIManager.GetPane(m_SubmodelsPanel).IsShown());
            break;
        case ID_MENU_VIEW_AUIPANE_DLOD_MODELS_LIST: UE.Check(m_AUIManager.GetPane(m_DlodModelsList).IsShown());
            break;
        case ID_MENU_VIEW_AUIPANE_TRANSFORM_DIALOG: UE.Check(m_AUIManager.GetPane(m_TransformDialog).IsShown());
            break;
    }
}
...
void ModelEditor::ChildFrameT::OnMenuModel(wxCommandEvent &CE) {
    switch (CE.GetId()) {
        ...
        case ID_MENU_MODEL_LOAD_SUBMODEL: {
            wxFileDialog FileDialog(this, "Load submodel", "", "", "Model files (*.*)|*.*",
...
void ModelEditor::ChildFrameT::OnMenuModelUpdate(wxUpdateUIEvent &UE) {
    ...
    switch (UE.GetId()) {
        case ID_MENU_MODEL_ANIM_PLAY: UE.Check(m_ModelDoc->GetAnimState().Speed != 0.0f);
            break;
        case ID_MENU_MODEL_ANIM_PAUSE: UE.Check(m_ModelDoc->GetAnimState().Speed == 0.0f);
            break;
        case ID_MENU_MODEL_UNLOAD_SUBMODELS: UE.Enable(m_ModelDoc->GetSubmodels().Size() > 0);
            break;
    }
}
...
void ModelEditor::ChildFrameT::OnClose(wxCloseEvent &CE) {
    ...
    wxMessageDialog Msg(NULL, "Save changes to model \"" + m_FileName + "\" before closing?", "CaWE Model Editor",
                        wxYES_NO | wxCANCEL);
