...
ResourcesT::ResourcesT(cf::CompositeConsoleT &CC, const std::string &ConsoleText, const GameInfoT &GameInfo,
                       MainWindowT &MainWin)
    ...
    switch (Options_ClientTextureDetail.GetValueInt()) {
        case 1: MatSys::TextureMapManager->SetMaxTextureSize(256);
            break;
        case 2: MatSys::TextureMapManager->SetMaxTextureSize(128);
            break;
    }
...
#ifdef _WIN32
        const std::string NullPaths[] = {
            std::string("Libs/") + QUOTE(SCONS_BUILD_DIR) + "/SoundSystem/SoundSysNull.dll",
            "./SoundSysNull.dll"
        };
#else
        const std::string NullPaths[]={
            std::string("Libs/")+QUOTE(SCONS_BUILD_DIR)+"/SoundSystem/libSoundSysNull.so",
            "./libSoundSysNull.so"
        };
#endif
...
#undef QUOTE_HELPER
    ...
    static ConVarT InitialMasterVolume("snd_InitialMasterVolume", 1.0,
                                       ConVarT::FLAG_MAIN_EXE | ConVarT::FLAG_PERSISTENT,
                                       "The master volume with which the sound system is initialized.", 0.0, 1.0);
    ...
    m_Client = new ClientT(MainWin, GameInfo, *m_ModelManager, *m_GuiResources);
    // The client initializes in IDLE state.
    ...
    IntrusivePtrT<cf::GuiSys::GuiImplT> MainMenuGui = cf::GuiSys::GuiMan->Find(
        "Games/" + GameInfo.GetName() + "/GUIs/MainMenu/MainMenu_main.cgui", true);
    ...
    m_SvGuiCallback->MainMenuGui = MainMenuGui;
    // This is the callback for the server, so that it can let the MainMenuGui know about its state changes.
    m_SvGuiCallback->OnServerStateChanged("idle");
    // Argh, this is a HACK for setting the initial state... can we move this / do it better?

    IntrusivePtrT<cf::GuiSys::GuiImplT> ConsoleGui = cf::GuiSys::GuiMan->Find(
        "Games/" + GameInfo.GetName() + "/GUIs/Console_main.cgui", true);
    IntrusivePtrT<cf::GuiSys::WindowT> ConsoleWindow = ConsoleGui != NULL
                                                           ? ConsoleGui->GetRootWindow()->Find("ConsoleOutput")
                                                           : NULL;
